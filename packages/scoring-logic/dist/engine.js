...
exports.getSegmentFromScore = void 0;
...
exports.calculateDemographicScore = calculateDemographicScore;
exports.checkHotLeadAlert = checkHotLeadAlert;
const shared_types_1 = require("@growth-engine/shared-types");
Object.defineProperty(exports, "getSegmentFromScore", { enumerable: true, get: function () { return shared_types_1.getSegmentFromScore; } });
const quiz_map_1 = require("./quiz-map");
// ─── Función principal ────────────────────────────────────────────────────────
/**
...
function calculateScore(request) {
    ...
    const segment = (0, shared_types_1.getSegmentFromScore)(total);
 ...
 */
function calculateQuizScore(answers, completionPct) {
    if (answers.length === 0)
        return 0;
    let rawScore = 0;
    let maxPossible = 0;
    for (const answer of answers) {
        ...
        const weight = quiz_map_1.QUESTION_WEIGHTS[qKey] ?? 1.0;
        const qMap = quiz_map_1.QUIZ_SCORING_MAP[qKey] ?? {};
...
function calculateEngagementScore(data) {
    let score = 0;
    if (data.askedPriceQuestion)
        score += 7;
    if (data.requestedDemo)
        score += 6;
...
function calculateDemographicScore(data) {
    ...
    if (quiz_map_1.HIGH_VALUE_INDUSTRIES.has(industry))
        score += 2;
 ...
 */
function checkHotLeadAlert(total, answers, _behavior) {
    if (total < 80)
        return false;
    const q9Urgent = answers.some((a) => a.questionIndex === 8 && (a.optionId === 'menos_1_mes' || a.optionId === 'ya'));
    const q10HasBudget = answers.some((a) => a.questionIndex === 9 && a.optionId !== 'no_tengo' && a.optionId !== null);
