...
let LeadsService = LeadsService_1 = class LeadsService {
    ...
    async captureFromWebhook(dto) {
        if (dto.externalId) {
            const existing = await this.prisma.withTenant(dto.tenantId, () => this.prisma.lead.findFirst({
                where: {
    ...
    async update(tenantId, leadId, dto) {
        await this.findOne(tenantId, leadId);
        return this.prisma.withTenant(tenantId, () => this.prisma.lead.update({
            where: { id: leadId },
            data: {
    ...
    async calculateAndUpdateScore(tenantId, leadId, scores, classification) {
        const totalScore = scores.quizScore + scores.behaviorScore + scores.engagementScore + scores.demographicScore;
        const segment = classification?.segment || (0, shared_types_1.getSegmentFromScore)(totalScore);
        const updated = await this.prisma.withTenant(tenantId, () => this.prisma.lead.update({
            where: { id: leadId },
            data: {
        ...
        if (totalScore >= 80) {
            await this.messagingQueue.add('hot-lead-alert', { leadId, tenantId, totalScore, segment }, { priority: 1 });
            const leadData = await this.prisma.withTenant(tenantId, () => this.prisma.lead.findFirst({
                where: { id: leadId },
    ...
    async updatePipelineStage(tenantId, leadId, newStage) {
    ...
    async getScore(tenantId, leadId) {
        const lead = await this.prisma.withTenant(tenantId, () => this.prisma.lead.findFirst({
            where: { id: leadId, tenantId, deletedAt: null },
            select: {
                quizScore: true,
    ...
    async getPrediction(tenantId, leadId) {
        const lead = await this.prisma.withTenant(tenantId, () => this.prisma.lead.findFirst({
            where: { id: leadId, tenantId, deletedAt: null },
            select: {
                id: true,
    ...
    async generateReport(tenantId, leadId) {
        const lead = await this.prisma.withTenant(tenantId, () => this.prisma.lead.findFirst({
            where: { id: leadId, tenantId, deletedAt: null },
            include: {
        ...
        const scoreBar = (score, label, color) => `<div style="margin-bottom:8px">
...
exports.LeadsService = LeadsService = LeadsService_1 = __decorate([
    ...
    __param(1, (0, inject_queue_decorator_1.InjectQueue)('scoring')),
